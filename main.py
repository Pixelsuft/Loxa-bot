...
from pyowm import OWM as NewOwm
from pyowm.commons.exceptions import NotFoundError as ErrorWeather
...
enable_rainbow = True
loha_chat = ['lox', 'loxa', 'лох', 'лоха', 'лохабот', 'лошара', 'боталох', 'ботолох', 'loxabot', 'loxa_bot', 'loxa-bot',
             'лох!', 'лоха!']
ctrl_c_is_exit = True
...
if '__DISCORD_BOT_TOKEN' in env:
    discord_bot_token = env['__DISCORD_BOT_TOKEN']
else:
    error('Discord BOT Token error')

if '__WEATHER_TOKEN' in env:
    weather_token = env['__WEATHER_TOKEN']
else:
    error('Weather Token error')

owm = NewOwm(weather_token).weather_manager()
...
colours_rainbow = [
    0x00ff00, 0xff0000, 0x0000ff, 0xffff00, 0xffffff, 0x000000, 0xff00ff, 0x00ffff,
    discord_color.dark_orange(), discord_color.orange(), discord_color.dark_gold(),
    discord_color.gold(), discord_color.dark_magenta(), discord_color.magenta(), discord_color.red(),
    discord_color.dark_red(), discord_color.blue(), discord_color.dark_blue(), discord_color.teal(),
    discord_color.dark_teal(), discord_color.green(), discord_color.dark_green(), discord_color.purple(),
    discord_color.dark_purple()
]
...
troll_block_words = ['Пшёл нахер', 'Лошара', 'А хер тебе с маслом, author!',
                     'author кабздец не повезло, я щас троллю сервера!', 'Лох, неудачник!']
                    ...
                    for i in split_cmd:
                        if entered:
                            ...
                            if i in hello_words:
                                entered = True
                                await ctx.send(
                                    str(random_choice(say_hello_words)).replace('author', message.author.mention))
...
async def mather(ctx, *, args_str):
    ...
    if len(cmd) > 0:
        if cmd[0] == 'eval':
            ...
            await ctx.send(str(random(int(cmd[1]), int(cmd[2]))))
        elif cmd[0] == 'pi':
            ...
            try:
                ...
                emb = discord_embed(title='Помощь по Stringer (' + str(prefix) + 'stringer КОМАНДА)',
                                    color=random_choice(colours_rainbow))
    ...
    if len(cmd) > 0:
        if cmd[0] == 'old_rsplit':
            ...
            try:
                ...
                emb.add_field(
                    name='old_split',
                    value="Разбитие текста на символы с помощью разделяющих символов и отправка разными сообщениями"
                )
    ...
    if len(args) > 0:
        if args[0] == 'open':
            ...
            if ctx.message.author.id == author_id:
                ...
                await ctx.send(str('Эта функция доступна только для автора.\nРади Защит от DDOS.'))
        elif args[0] == 'rotate':
            try:
            ...
            try:
                user_images[str(ctx.message.author.id)] = user_images[str(ctx.message.author.id)].crop(
                    (int(args[1]), int(args[2]), int(args[3]), int(args[4])))
            except:
                await ctx.send(error_text)
        elif args[0] == 'filter':
            try:
                user_images[str(ctx.message.author.id)] = user_images[str(ctx.message.author.id)].filter(
                    eval('pillow_filter.' + str(args[1])))
            except:
                await ctx.send(error_text)
        elif args[0] == 'convert':
            try:
            ...
            try:
                user_images[str(ctx.message.author.id)] = user_images[str(ctx.message.author.id)].resize(
                    (int(args[1]), int(args[2])), pillow_image.ANTIALIAS)
            except:
                await ctx.send(error_text)
        elif args[0] == 'size':
            try:
            ...
            try:
                ...
                emb = discord_embed(title='Помощь по Imaginer (' + str(prefix) + 'imaginer КОМАНДА)',
                                    color=random_choice(colours_rainbow))
                emb.add_field(name='help', value="Открытие этой подсказки")
        ...
        await ctx.send(
            ctx.message.author.mention + ', для большей информации наберите ' + str(prefix) + 'imaginer help.')
